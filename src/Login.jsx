      ...
      <div className="w-full bg-[rgba(230,243,255,0.75)] flex flex-row h-auto min-h-screen relative overflow-hidden p-4 md:h-[1024px]">
        ...
        <form onSubmit={handleSubmit} className="bg-[#fcfeff] rounded-[25px] w-full md:w-[60%] md:h-[1024px] p-6 flex flex-col items-center">
          ...
          <div className="flex flex-row justify-start mt-6">
            <p className="text-[16px] text-[#7c838a]">Forgot password?</p>
            <Link to="/forgotpasswordform" className="ml-1 text-[#d57317]">Reset</Link>
          </div>
          <button type='submit' className="w-full md:w-[340px] bg-[#f9ed32] text-[#000000] rounded-[10px] mt-8 p-3 font-medium text-lg flex items-center justify-center">
            {loading ? <span className="animate-spin border-2 border-black border-t-transparent rounded-full w-5 h-5"></span> : 'Login'}
          </button>
          <div className="flex flex-row justify-start mt-6">
            <p className="text-[16px] text-[#7c838a]">Don't have an account?</p>
            <Link to="/app/register" className="ml-1 text-[#f9ed32]">Register</Link>
          </div>
        </form>
      </div>
      <MobileFooter />
